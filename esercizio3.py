...
def merge(list1,list2):
    ...
    while(i<list1.__len__() and e<list2.__len__() ):
        if(current_first_list._element>current_second_list._element):
            list1.add_before(list1.make_position(current_first_list),current_second_list._element)
            current_second_list=current_second_list._next
            e+=1
        else:
            current_first_list=current_first_list
            i+=1
    while (e<list2.__len__()):
        list1.add_before(list1.make_position(current_first_list),current_second_list.element())
