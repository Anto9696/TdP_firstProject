...
def merge(list1, list2):
    ...
    while i < len(list1) and e < len(list2):
        if current_first_list.element() > current_second_list.element():
            ...
            current_second_list = super(CircularPositionalList, list2).after(current_second_list)  #list2._next_position(current_second_list)
            e += 1
        else:
            new_list.add_last(current_first_list.element())
            current_first_list = super(CircularPositionalList, list1).after(current_first_list) #list1._next_position(current_first_list)
            i += 1
    while i<len(list1):
        new_list.add_last(current_first_list.element())
        current_first_list = super(CircularPositionalList, list1).after(current_first_list)   #list1._next_position(current_first_list)
